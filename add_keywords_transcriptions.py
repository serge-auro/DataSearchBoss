...
# Загрузка JSON-файла
try:
    with open('whisper_transcriptions/transcriptions_1.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
...
for index, (key, video_info) in enumerate(data.items(), start=1):
    transcriptions = video_info.get("transcription", "")
    video_url = video_info.get("url", "")
    key_words = video_info.get("keywords", None)

    if transcriptions and not key_words:
        start_extraction_time = time.time()
        key_words = extract_keywords(transcriptions)
        ...
        logging.info(f"Транскрипция: {transcriptions}")
...
try:
    with open('whisper_transcriptions/new_transcriptions_1.json', 'w', encoding='utf-8') as f:
...
logging.info(f"Обработка завершена. Обновленный файл сохранен как 'new_transcriptions_1.json'.")
