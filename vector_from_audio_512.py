...
# Чтение 500 записей из файла all_videos.json
with open("video_description/all_videos.json", "r", encoding="utf-8") as file:
    ...
    """
    На момент 13.06.2024 23:00 обработаны видеофайлы 0 - 399 (включительно).
    Для дальнейшей работы задать start_video = 400, end_video = 401 (минимум)
    """

new_audio_vectors_512 = {}
new_audio_process_512 = {}
new_audio_fail_512 = {}
...
async def encode_audio(request: AudioEncodeRequest):
    ...
    try:
        ...
        # Преобразование вектора в нужный формат 1x512
...
async def process_records(records):
    ...
    for key, value in process_records:
        ...
        try:
            ...
            new_audio_vectors_512[key] = result["features"]
            new_audio_process_512[key] = {
            ...
            new_audio_fail_512[key] = {"url": video_url, "error":  f"{error_name}: {error_description}"}
    return new_audio_vectors_512, new_audio_process_512, new_audio_fail_512

program_start_time = time.time()

new_audio_vectors_512, new_audio_process_512, new_audio_fail_512 = asyncio.run(process_records(n100_records))

# Чтение существующих данных из файла audio_vectors_512
try:
    with open("audio_processing/audio_vectors_512.json", "r") as file:
        audio_vectors_512 = json.load(file)
except FileNotFoundError:
    audio_vectors_512 = {}
# Дополнение данных новыми записями
audio_vectors_512.update(new_audio_vectors_512)
# Запись обновленных данных обратно в файл audio_vectors_512
with open("audio_processing/audio_vectors_512.json", "w") as file:
    json.dump(audio_vectors_512, file, indent=4)

# Чтение существующих данных из файла audio_process_512
try:
    with open("audio_processing/audio_process.json_512", "r") as file:
        audio_process_512 = json.load(file)
except FileNotFoundError:
    audio_process_512 = {}
# Дополнение данных новыми записями
audio_process_512.update(new_audio_process_512)
# Запись обновленных данных обратно в файл audio_process_512
with open("audio_processing/audio_process_512.json", "w") as file:
    json.dump(audio_process_512, file, indent=4)

# Чтение существующих данных из файла audio_fail_512
try:
    with open("audio_processing/audio_fail.json_512", "r") as file:
        audio_fail_512 = json.load(file)
except FileNotFoundError:
    audio_fail_512 = {}
# Дополнение данных новыми записями
audio_fail_512.update(new_audio_fail_512)
# Запись обновленных данных обратно в файл audio_fail_512
with open("audio_processing/audio_fail_512.json", "w") as file:
    json.dump(audio_fail_512, file, indent=4)
